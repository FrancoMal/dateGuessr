...
export class ScoreService {
  ...
  private load(): ScoreData {
    try {
      ...
      if (raw) {
        return { score: 0, streak: 0, bestStreak: 0, totalAnswered: 0, totalCorrect: 0, ...JSON.parse(raw) };
      }
    } catch {
  ...
  private save(): void {
    safeSetItem(this.STORAGE_KEY, JSON.stringify(this.data));
  }
  ...
  /** Reinicia la sesión preservando la mejor racha (acción por defecto del botón "Reiniciar"). */
  resetSession(): void {
    this.data = { score: 0, streak: 0, bestStreak: this.data.bestStreak, totalAnswered: 0, totalCorrect: 0 };
    this.save();
  }

  /** Borra TODO, incluida la mejor racha (acción secundaria, siempre con confirmación). */
  reset(): void {
